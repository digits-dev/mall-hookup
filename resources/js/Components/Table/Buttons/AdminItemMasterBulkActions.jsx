...
const AdminItemMasterBulkActions = ({disabled, setData, onConfirm}) => {
  ...
  return (
    <>
        <div ref={notifRef} className={`z-[60] select-none relative inline-block text-left font-poppins ${disabled ? 'opacity-50' : 'opacity-100' }`}>
            ...
            >
                <div className={`flex items-center gap-2 px-3 py-2 cursor-pointer ${theme === 'bg-skin-black' ? 'hover:bg-black/20' : 'hover:bg-gray-200'}`} onClick={()=>{handleMenuToggle(); handleModalToggle(); setBulkAction('Active'); setData('bulkAction', 'Active');}}>
                    <i className={`fa-solid fa-circle-check text-green-500 text-wrap text-[8px] md:text-xs`}></i>
                    <p className='text-[8px] md:text-[10px] font-medium text-nowrap'>Set SKU Active</p>
                </div>
                <div className={`flex items-center gap-2 px-3 py-2 cursor-pointer ${theme === 'bg-skin-black' ? 'hover:bg-black/20' : 'hover:bg-gray-200'}`} onClick={()=>{handleMenuToggle(); handleModalToggle(); setBulkAction('EOL-Digits'); setData('bulkAction', 'EOL-Digits');}}>
                    <i className={`fa-solid fa-circle-check text-green-500 text-wrap text-[8px] md:text-xs`}></i>
                    <p className='text-[8px] md:text-[10px] font-medium text-nowrap'>Set SKU EOL-Digits</p>
                </div>
                <div className={`flex items-center gap-2 px-3 py-2 cursor-pointer ${theme === 'bg-skin-black' ? 'hover:bg-black/20' : 'hover:bg-gray-200'}`} onClick={()=>{handleMenuToggle(); handleModalToggle(); setBulkAction('EOL-Supplier'); setData('bulkAction', 'EOL-Supplier');}}>
                    <i className={`fa-solid fa-circle-check text-green-500 text-wrap text-[8px] md:text-xs`}></i>
                    <p className='text-[8px] md:text-[10px] font-medium text-nowrap'>Set SKU EOL-Supplier</p>
                </div>
                <div className={`flex items-center gap-2 px-3 py-2 cursor-pointer ${theme === 'bg-skin-black' ? 'hover:bg-black/20' : 'hover:bg-gray-200'}`} onClick={()=>{handleMenuToggle(); handleModalToggle(); setBulkAction('Invalid'); setData('bulkAction', 'Invalid');}}>
                    <i className={`fa-solid fa-circle-xmark text-red-500 text-wrap text-[8px] md:text-xs`}></i>
                    <p className='text-[8px] md:text-[10px] font-medium '>Set SKU Invalid</p>
                </div>
        ...
        <div className={`${theme === 'bg-skin-black' ? 'bg-black-table-color text-gray-300' : 'bg-white'} p-5 max-w-[30rem] w-full m-5 rounded-lg shadow-lg duration-300 ease-in-out ${isModalOpen ? 'opacity-100 scale-100' : 'opacity-0 scale-95 pointer-events-none'}`}>
            ...
            <>
                ...
                <p className={`${theme == 'bg-skin-black' ? 'text-white' : 'text-black'} text-sm text-gray-500 mb-7`}>Set SKU Status of selected item/s to {bulkAction}?</p>
...
export default AdminItemMasterBulkActions